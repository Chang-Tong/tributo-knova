...
| Training | Ray-native ClickHouse input and two-worker official distributed XGBoost | Local integration smoke |
...
- The pinned official boosting package persists a checkpoint but explicitly
  ...
  gate.
- The official XGBoost execution result currently supplies no terminal
  evaluation metrics. KnoVa reports an empty metric list and does not fabricate
  AUC/F1 or a misleading `metrics.json`.
